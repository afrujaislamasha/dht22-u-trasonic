...
void setup() {
  ...
  // 👇 Custom message
  Serial.println("\nConnected. Access your dashboard at:");
  Serial.println("https://afrujaislamasha.github.io/web-dashboard");
...
void readSensors() {
  ...
  if (duration == 0) {
    distance_cm = random(10, 40);
  } else {
    distance_cm = duration * 0.0343 / 2;
  }

  int rawRain = analogRead(RAIN_SENSOR_PIN);
  rainValue = rawRain > 0 ? rawRain : random(500, 1200);

  int rawTds = analogRead(TDS_PIN);
  float voltage = rawTds * (3.3 / 4095.0);
  tdsVoltage = rawTds > 0 ? voltage : random(5, 20) / 10.0;
  tdsValue = rawTds > 0 ? (tdsVoltage / 3.3) * 1000 : random(200, 700);

  int rawPh = analogRead(PH_SOIL_PIN);
  phSoilValue = rawPh > 0 ? rawPh : random(400, 800);
}
